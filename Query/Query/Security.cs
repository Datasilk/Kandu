...
namespace Query
{
    public static class Security
    {
        public static void SaveKeys(int orgId, int userId, Dictionary<string, bool> keys)
        {
            ...
            Sql.ExecuteNonQuery("OrgSecurity_SaveKeys", new { orgId, userId, keys = Common.Serializer.ToXmlDocument(list).OuterXml });
        }

        public static List<Models.SecurityKey>  ForUser (int orgId, int userId)
        {
            return Sql.Populate<Models.SecurityKey>("OrgSecurity_ForUser", new { orgId, userId });
        }
